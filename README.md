...
Due to the dynamic nature of this script, it could be used to analyze any local, state, or national election. As long as the dataset contains a ballot ID, location, and the name of the candidate voted for, this script will work to analyze any election, even if there are a different number of locations (cities/counties/states) or candidates participating in the election. To analyze a national election, one would only need to replace the county names with states (and change variable names), and the analysis would run the same. By not directly tying the code to our current dataset, it is possible to use this script to analyze any election given a Ballot ID, County (or state), and the candidate who was voted for. 
